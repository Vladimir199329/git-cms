...
{capture name="buttons"}
    {if $id}
        {capture name="tools_list"}
            <li>{btn type="list" text=__("delete") class="cm-confirm" href="departaments.delete_departament?departament_id=`$id`" method="POST"}</li>
        {/capture}
        {dropdown content=$smarty.capture.tools_list}
    {/if}
    {if !$id}
        {include file="buttons/save_cancel.tpl" but_role="submit-link" but_target_form="departaments_form" but_name="dispatch[departaments.update_departament]"}
    {else}
        {include file="buttons/save_cancel.tpl" but_name="dispatch[departaments.update_departament]" but_role="submit-link" but_target_form="departaments_form" hide_first_button=$hide_first_button hide_second_button=$hide_second_button save=$id}
    {/if}
{/capture}
